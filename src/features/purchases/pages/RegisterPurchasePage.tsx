...
import {
  ...
  Package, Receipt, Banknote, ArrowRightLeft, Clock, Calendar, User, Check,
} from 'lucide-react';
import { ProductLineItem, ProductLine } from '../components/PurchaseLineItem';
...
export default function RegisterPurchasePage() {
  ...
  const handleRemoveProduct = (id: string) => {
    if (productLines.length > 1) setProductLines(productLines.filter(p => p.id !== id));
  };
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 pb-24 lg:pb-12">
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 dark:text-slate-100">Registrar compra</h1>
            <p className="text-slate-500 dark:text-slate-400 mt-1">Agrega productos o gastos adquiridos</p>
          </div>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            ...
            <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Fecha de compra</label>
                ...
                <div className="sm:col-span-2">
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Tipo de compra</label>
                  <div className="grid grid-cols-2 gap-3 p-1 bg-slate-100 dark:bg-slate-700 rounded-xl">
                    {[
                      { id: 'inventory', label: 'Producto para inventario', icon: Package },
                      { id: 'expense',   label: 'Gasto (servicios, insumos)', icon: Receipt },
                    ].map(t => (
                      <button key={t.id} type="button" onClick={() => setPurchaseType(t.id as 'inventory' | 'expense')}
                        className={`flex items-center justify-center gap-2 py-2.5 px-4 rounded-lg text-sm font-medium transition-all duration-200 ${purchaseType === t.id ? 'bg-white dark:bg-slate-600 text-blue-600 shadow-sm ring-1 ring-black/5' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}>
            ...
            <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: 0.1 }}
              ...
              <div className="flex items-center justify-between mb-6 pb-4 border-b border-slate-100 dark:border-slate-700">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-lg font-bold text-slate-900 dark:text-slate-100">Productos</h2>
                </div>
                <span className="bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 text-xs font-bold px-2.5 py-1 rounded-full">{productLines.length}</span>
              </div>
              ...
              <div className="space-y-1">
                <AnimatePresence initial={false}>
                  {productLines.map((item, index) => (
                    <ProductLineItem key={item.id} item={item} index={index}
                      onChange={handleProductChange} onRemove={handleRemoveProduct} canRemove={productLines.length > 1} />
                  ))}
                </AnimatePresence>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {[
                  ...
                  <button key={method.id} type="button" onClick={() => setPaymentMethod(method.id as any)}
                    ...
                    {'note' in method && method.note && <span className="text-[10px] mt-1 text-center opacity-75 leading-tight">{method.note}</span>}
                  </button>
                ))}
              </div>
            </motion.section>
          ...
          <div className="lg:col-span-1">
            <PurchaseSummary totalAmount={totalAmount} totalItems={productLines.length}
              totalUnits={totalUnits} paymentMethod={paymentMethod} purchaseType={purchaseType} />
          </div>
        </div>
      </main>
